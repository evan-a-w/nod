...
let%test_unit "x86 peephole: swap via temp" =
  let items =
    [ Asm.Instr (Mov (Reg Reg.r10, Reg Reg.rax))
    ; Asm.Instr (Mov (Reg Reg.rax, Reg Reg.rbx))
    ; Asm.Instr (Mov (Reg Reg.rbx, Reg Reg.r10))
    ]
  in
  let expected =
    [ Asm.Instr (Xchg (Reg Reg.rax, Reg Reg.rbx))
    ; Asm.Instr (Mov (Reg Reg.r10, Reg Reg.rbx))
    ]
  in
...
let%test_unit "x86 peephole: forward stack store/load" =
  let addr = Mem (Reg.rsp, -8) in
  let items =
    [ Asm.Instr (Mov (addr, Reg Reg.rax)); Asm.Instr (Mov (Reg Reg.rbx, addr)) ]
  in
  let expected =
    [ Asm.Instr (Mov (addr, Reg Reg.rax))
    ; Asm.Instr (Mov (Reg Reg.rbx, Reg Reg.rax))
    ]
  in
  let optimized = optimize items in
  if not (List.equal Asm.equal_item optimized expected)
...
let%test_unit "x86 peephole: push/pop to mov" =
  let items = [ Asm.Instr (Push (Reg Reg.rax)); Asm.Instr (Pop Reg.rbx) ] in
  let expected = [ Asm.Instr (Mov (Reg Reg.rbx, Reg Reg.rax)) ] in
  let optimized = optimize items in
  if not (List.equal Asm.equal_item optimized expected)
  then raise_s [%message (optimized : Asm.item list) (expected : Asm.item list)]
;;
