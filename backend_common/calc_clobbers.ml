  ...
  let calc_edges (functions : Function.t String.Map.t) =
    ...
    Map.iter functions ~f:(fun function_ ->
      ...
      Block.iter_instructions function_.root ~f:(fun ir ->
        this_defs := Set.union !this_defs (arch_reg_defs ir |> Reg.Set.of_list);
        on_arch_irs ir ~f:(fun ir ->
          match Arch_ir.fn ir with
          | None -> ()
          | Some fn -> uses_fn function_.name fn));
      ...
      Hashtbl.update to_restore fn.name ~f:(function
        ...
        | None ->
          Set.inter
            ...
            (all_callee_saved ~call_conv:fn.call_conv));
          ...
          (Hashtbl.find_exn defs fn.name
           :: (Hashtbl.find_exn edges fn.name
               ...
               |> List.map ~f:(fun fn' ->
                 ...
                 (* CR-soon: This won't work for extern functions, where we need to know a callconv and assume everything is clobbered. These don't exist yet. *)
                 |> Option.value ~default:Reg.Set.empty)))
      in
      let new_clobbers =
        Set.diff new_clobbers_raw (all_callee_saved ~call_conv:fn.call_conv)
      in
