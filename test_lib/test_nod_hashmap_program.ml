...
let hash_fn =
  [%nod fun (key : int64) (capacity : int64) ->
...
let root =
  let instrs =
    [%nod
      ...
      seq [ store_addr (lit 99L) query_miss 0; store_addr (lit 5L) query_miss 8 ];
      let%named miss = hashmap_get state query_miss in
      let%named total0 = add hit miss in
      let%named total1 = add total0 init_done in
      let%named total2 = sub total1 init_done in
      let%named total3 = add total2 put1 in
      let%named total4 = sub total3 put1 in
      let%named total5 = add total4 put2 in
      let%named total6 = sub total5 put2 in
      return total6]
  in
      ...
      ((%entry (args ())
        (instrs
         ((Alloca ((dest ((name state) (type_ Ptr))) (size (Lit 16))))
          ...
          (Add
           ((dest ((name total0) (type_ I64)))
            (src1 (Var ((name hit) (type_ I64))))
            (src2 (Var ((name miss) (type_ I64))))))
          (Add
           ((dest ((name total1) (type_ I64)))
            (src1 (Var ((name total0) (type_ I64))))
            (src2 (Var ((name init_done) (type_ I64))))))
          (Sub
           ((dest ((name total2) (type_ I64)))
            (src1 (Var ((name total1) (type_ I64))))
            (src2 (Var ((name init_done) (type_ I64))))))
          (Add
           ((dest ((name total3) (type_ I64)))
            (src1 (Var ((name total2) (type_ I64))))
            (src2 (Var ((name put1) (type_ I64))))))
          (Sub
           ((dest ((name total4) (type_ I64)))
            (src1 (Var ((name total3) (type_ I64))))
            (src2 (Var ((name put1) (type_ I64))))))
          (Add
           ((dest ((name total5) (type_ I64)))
            (src1 (Var ((name total4) (type_ I64))))
            (src2 (Var ((name put2) (type_ I64))))))
          (Sub
           ((dest ((name total6) (type_ I64)))
            (src1 (Var ((name total5) (type_ I64))))
            (src2 (Var ((name put2) (type_ I64))))))
          (Return (Var ((name total6) (type_ I64)))))))))
...
let compile_and_execute_program_exn program expected =
  List.iter test_architectures ~f:(fun arch ->
    ...
    in
    let output = execute_asm ~arch ~system:host_system ~harness:harness_source asm in
