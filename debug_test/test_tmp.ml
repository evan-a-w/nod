        ...
        ((%root (args ())
          (instrs
           (((id (Instr_id 1))
            ...
            ((id (Instr_id 3))
             (ir
              (Call (callee (Direct child)) (results ((Value_id 0)))
               (args ((Var (Value_id 1)))))))
            ((id (Instr_id 0)) (ir (Return (Var (Value_id 0))))))))))
...
let%expect_test "debug borked opt ssa" =
  ...
  [%expect
    ...
    (%root
     (instrs
      ((Call (callee (Direct sum8)) (results (((name res) (type_ I64))))
         ...
         (instrs
          (((id (Instr_id 13))
           ...
           ((id (Instr_id 5))
            (ir
             (Arm64
              (Call (callee (Direct sum8)) (results (((reg X0) (class_ I64))))
               (args
...
let%expect_test "debug borked" =
  ...
  | Ok program ->
    ...
    [%expect
      ...
      +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------------------------+
      | (Arm64(Call(callee(Direct sum8))(results(((reg(Allocated((name tmp_force_physical)(type_ I64))(X0)))(class_ I64))))(args((Imm 1)(Imm 2)(Imm 3)(Imm 4)(Imm 5)(Imm 6)(Imm 7)(Imm 8))))) | (((name tmp_force_physical)(type_ I64))) |
      +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------------------------+
